...
import { getSermons, deleteSermon } from './actions'

export default async function SermonsList() {
    const { sermons, error } = await getSermons()

    if (error) {
        return <div className="p-8 text-red-500">Error loading sermons.</div>
    }

    return (
        <div className="flex-1 space-y-4 p-8 pt-6">
            <div className="flex items-center justify-between space-y-2">
                <h2 className="text-3xl font-bold tracking-tight">Manage Sermons</h2>
                <div className="flex items-center space-x-2">
                    <Link href="/admin/sermons/create" className="bg-slate-900 text-white hover:bg-slate-900/90 h-10 px-4 py-2 inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium transition-colors">
                        Add New Sermon
                    </Link>
                </div>
            </div>
            ...
            <div className="rounded-md border">
                <table className="w-full caption-bottom text-sm">
                    ...
                    <tbody className="[&_tr:last-child]:border-0">
                        {sermons?.length === 0 ? (
                            <tr>
                                <td colSpan={4} className="p-4 text-center text-muted-foreground">No sermons found.</td>
                            </tr>
                        ) : (
                            sermons?.map((sermon) => (
                                <tr key={sermon.id} className="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                    ...
                                    <td className="p-4 align-middle text-right space-x-2">
                                        <Link href={`/admin/sermons/${sermon.id}/edit`} className="text-blue-500 hover:underline">Edit</Link>
                                        <form action={async () => {
                                            'use server'
